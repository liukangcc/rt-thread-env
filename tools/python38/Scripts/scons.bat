...
python -c "from os.path import join; import sys; sys.path = [ join(sys.prefix, 'Lib', 'site-packages', 'SCons'), join(sys.prefix, 'Lib', 'site-packages', 'SCons'), join(sys.prefix, 'SCons'), join(sys.prefix, 'SCons')] + sys.path; import SCons.Script; SCons.Script.main()" %1 %2 %3 %4 %5 %6 %7 %8 %9
