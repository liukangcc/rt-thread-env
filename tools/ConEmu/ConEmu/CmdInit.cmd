...
if /i "%processor_architecture%"=="x86" (
        set PYTHONPATH=%ENV_ROOT%\tools\Python38_32
        set PYTHONHOME=%ENV_ROOT%\tools\Python38_32
) else if /i "%processor_architecture%"=="amd64" (
    if defined processor_architew6432 (
        set PYTHONPATH=%ENV_ROOT%\tools\Python38_32
        set PYTHONHOME=%ENV_ROOT%\tools\Python38_32
    ) else (
        set PYTHONPATH=%ENV_ROOT%\tools\Python38
        set PYTHONHOME=%ENV_ROOT%\tools\Python38
    )
)
...
chcp 65001 > nul
echo 欢迎使用 RT-Thread env (V1.3.0) 工具
echo  ^\ ^| /
